...
export default async function EditEmployeePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: employee }, { data: departments }, { data: employees }] = await Promise.all([
    supabase
      .from("employees")
      .select(
        "id, employee_code, full_name, work_email, phone_number, department_id, position, manager_id, join_date, role, status, auth_user_id, must_change_password, created_at, updated_at",
      )
      .eq("id", id)
      .single(),
