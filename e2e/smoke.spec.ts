...
test.describe("Smoke Tests", () => {
  ...
  test("app loads - login page renders", async ({ page }) => {
    ...
    await expect(page.getByRole("heading").or(page.locator("form")).first()).toBeVisible({ timeout: 30000 });
  });
  ...
  for (const { name, path } of adminPages) {
    test(`page loads as admin: ${name} (${path})`, async ({ page }) => {
      ...
      await expect(page.locator("body")).not.toBeEmpty();
      await expect(page.locator('main, [role="main"], h1, h2, h3, .content, #content').first()).toBeVisible({
        timeout: 30000,
      });
  ...
  test("sidebar renders with nav items", async ({ page }) => {
    ...
    const sidebar = page.locator('nav, aside, [role="navigation"], .sidebar, #sidebar').first();
  ...
  test("logout works", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /system administrator|admin@company\.com/i }).click();
